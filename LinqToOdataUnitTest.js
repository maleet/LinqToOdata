describe("Metadata", function () {
    it("Create object graph by metadata.", function () {
        runs(function () {
            ...
            expect(JSON.stringify(bpBuyFrom)).toEqual("jsonString");
        });
    });
});
...
function create(metadata, item) {
    ...
    return parseMetadataToObject(metadata, key);
}

function parseMetadataToObject(metadata, key){
    var item = metadata[key];

    var deep = 10;
    deep--;
    var type = jQuery.extend({}, item);
...
function parseMetadataForOdataQueries(metadata) {
    var processed = {};
    Object.keys(service.schema).forEach(function (key) {
        if (processed[key] === undefined) {
            processed[key] = initialize(key);
        }
    });

    Object.keys(processed).forEach(function (key) {
        initializeChildren(key, processed[key]);
    });

    Object.keys(processed).forEach(function (key) {
        processed[key] = JSON.decycle(processed[key]);
    });

    Object.keys(processed).forEach(function (key) {
        var obj = processed[key];
        fixCircularReference(obj);
        /*if (circularErrors.length) {
         console.log(circularErrors);
         }*/
    });
}
